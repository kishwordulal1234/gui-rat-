...
def generate_payload():
    choice = os_choice.get()
    payload_type = payload_choice.get()
    ...
    encode_flag = ""
    if encoding_var.get():
        encoding_choice = encoding_type.get()
...
tk.Checkbutton(app, text="Use encoding?", variable=encoding_var).grid(row=5, column=0, sticky=tk.W, pady=2)
